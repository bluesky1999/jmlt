...
public class SVMStructLearn {
	...
	private static Logger logger = Logger.getLogger(SVMStructLearn.class);
	
	public SVMStructLearn()
	{
	...
	public void svm_learn_struct(SAMPLE sample, STRUCT_LEARN_PARM sparm,
		...
		if (kparm.kernel_type !=  ModelConstant.LINEAR) {
			kcache = SVMLearn.kernel_cache_init(Math.max(cset.m, 1),
		...
		SVMCommon.add_weight_vector_to_linear_model(svmModel);
		...
		rt_init += Math.max(SVMCommon.get_runtime() - rt1, 0);
		rt_total += Math.max(SVMCommon.get_runtime() - rt1, 0);
				...
				do { 
					...
					for (i = 0; i < n; i++) {
						...
						if ((use_shrinking == 0) || (opti[i] != opti_round)) {
							...
							rt_viol += Math.max(SVMCommon.get_runtime() - rt2,
									0);
							...
							rt2 = SVMCommon.get_runtime();
							if (fycache != null) {
								fy = SVMCommon.copy_svector(fycache[i]);
							} else {
...
;
							rt_psi += Math.max(SVMCommon.get_runtime() - rt2,
									0);
							...
							SVMCommon.append_svector_list(fy, fybar);
							doc = SVMCommon.create_example(cset.m, 0, i + 1,
									1, fy);
							...
							for (j = 0; j < cset.m; j++)
								if (cset.lhs[j].slackid == i + 1) {
									if (sparm.slack_norm == 2)
										slack = Math.max(slack,cset.rhs[j]- (SVMCommon.classify_example(svmModel,cset.lhs[j]) - sm.w[sizePsi+ i]/ (Math.sqrt(2 * svmCnorm))));
									else
										slack = Math.max(slack,cset.rhs[j]- SVMCommon.classify_example(svmModel,cset.lhs[j]));
								}

							//if `error' add constraint and recompute 
							dist = SVMCommon.classify_example(svmModel, doc);
							...
							if ((dist + slack) < (margin - epsilon)) {
								...
								cset.m++;
								SVMCommon.realloc(cset);

								if (kparm.kernel_type ==  ModelConstant.LINEAR) {
									diff = SVMCommon.add_list_ss(fy);
									if (sparm.slack_norm == 1)
										cset.lhs[cset.m - 1] = SVMCommon.create_example(cset.m - 1,0,i + 1,1,SVMCommon.copy_svector(diff));
									else if (sparm.slack_norm == 2) {
										...
										slackvec = SVMCommon.create_svector(slackv, null, 1.0);
										cset.lhs[cset.m - 1] = SVMCommon.create_example(cset.m - 1, 0,i + 1, 1,SVMCommon.add_ss(diff,slackvec));
									}
								} else {//kernel is used 
									if (sparm.slack_norm == 1)
										cset.lhs[cset.m - 1] = SVMCommon
								...
								SVMCommon.realloc_rhs(cset);
								cset.rhs[cset.m - 1] = margin;

								alpha_g = SVMCommon.realloc_alpha(alpha_g,
										cset.m);
								alpha_g[cset.m - 1] = 0;
								alphahist_g = SVMCommon.realloc_alpha_list(alphahist_g, cset.m);
						...
						if ((newconstraints >= sparm.newconstretrain)
							...
							rt_opt += Math.max(SVMCommon.get_runtime() - rt2,
									0);
		...
		if (SVMStructCommon.struct_verbosity >= 1) {
			...
			if (sparm.slack_norm == 1) {
				for (j = 0; j < cset.m; j++)
					slacks[cset.lhs[j].slackid] = Math.max(
							...
							cset.rhs[j]
									- SVMCommon.classify_example(svmModel,
											cset.lhs[j]));
			} else if (sparm.slack_norm == 2) {
				for (j = 0; j < cset.m; j++)
					slacks[cset.lhs[j].slackid] = Math.max(
			...
			modellength = SVMCommon.model_length_s(svmModel);
		...
		if (SVMStructCommon.struct_verbosity >= 4)
			logger.info(SVMStructCommon.printW(sm.w, sizePsi, n, lparm.svm_c));
	...
	public void svm_learn_struct_joint(SAMPLE sample, STRUCT_LEARN_PARM sparm,
		...
		if ((alg_type == SVMStructCommon.ONESLACK_DUAL_ALG)
				|| (alg_type == SVMStructCommon.ONESLACK_DUAL_CACHE_ALG))
		...
		for (i = 0; i < n; i++) {
			if ( ModelConstant.USE_FYCACHE != 0) {
				...
				if (kparm.kernel_type ==  ModelConstant.LINEAR) {
					diff = SVMCommon.add_list_sort_ss_r(fy,
							SVMStructCommon.COMPACT_ROUNDING_THRESH);
					fy = diff;
				}
			} else
				fy = null;
		...
		if (kparm.kernel_type ==  ModelConstant.LINEAR) {
			...
			lhs_n = SVMCommon.create_nvector(sm.sizePsi);
		}
		// randomize order or training examples 
		if (batch_size < n)
			randmapping = SVMCommon.random_order(n);
		...
		do { // iteratively find and add constraints to working set 
			...
			for (j = 0, slack = -1; (j < cset.m) && (slack == -1); j++) {
				...
				if (alpha_g[j] > alphasum / cset.m) {
					slack = Math.max(0,cset.rhs[j]- SVMCommon.classify_example(svmModel,cset.lhs[j]));
				}
			}
			...
			if (alg_type == ONESLACK_DUAL_CACHE_ALG) {
				...
				if (SVMStructCommon.struct_verbosity >= 2)
					rt_cacheupdate += Math.max(SVMCommon.get_runtime() - rt2,
							0);
				...
				if (viol - slack > Math.max(epsilon_est / 10, sparm.epsilon)) {
					...
					if (SVMStructCommon.struct_verbosity >= 2)
						rt_cacheconst += Math.max(SVMCommon.get_runtime()
								- rt2, 0);
					cached_constraint = 1;
				} else {
					...
					for (j = 0; (j < batch_size)
						...
						if (SVMStructCommon.struct_verbosity >= 2)
							rt_cacheadd += Math.max(SVMCommon.get_runtime()
									- rt2, 0);
					...
					if ((SVMStructCommon.struct_verbosity >= 1) && (j != n))
				...
				cached_constraint = 0;
				if (kparm.kernel_type ==  ModelConstant.LINEAR)
					SVMCommon.clear_nvector(lhs_n, sm.sizePsi);
				SVMCommon.progress_n = 0;
				rt_total += Math.max(SVMCommon.get_runtime() - rt1, 0);
				...
				for (i = 0; i < n; i++) {
					...
					if (kparm.kernel_type ==  ModelConstant.LINEAR) {
						SVMCommon.add_list_n_ns(lhs_n, fydelta_g, 1.0);
					} else {
						SVMCommon.append_svector_list(fydelta_g, lhs_g);
				...
				if (kparm.kernel_type ==  ModelConstant.LINEAR) {
					lhs_g = SVMCommon.create_svector_n_r(lhs_n, sm.sizePsi,
							null, 1.0,
							SVMStructCommon.COMPACT_ROUNDING_THRESH);
				}
				doc = SVMCommon.create_example(cset.m, 0, 1, 1, lhs_g);
				lhsXw = SVMCommon.classify_example(svmModel, doc);
			...
			if ((ceps > sparm.epsilon) || cached_constraint != 0) {
				...
				int ti = 0;
				ti = cset.m + 1;
				cset.lhs = SVMCommon.reallocDOCS(cset.lhs, ti);

				cset.lhs[cset.m] = SVMCommon.create_example(cset.m, 0, 1, 1,
						lhs_g);
				...
				alpha_g = SVMCommon.reallocDoubleArr(alpha_g, cset.m + 1);
				alpha_g[cset.m] = 0;
				// alphahist = new int[cset.m + 1];
				alphahist_g = SVMCommon.reallocIntArr(alphahist_g,
						cset.m + 1);
				...
				if ((alg_type == ONESLACK_DUAL_ALG)
					...
					if (SVMStructCommon.struct_verbosity >= 2)
						rt_kernel += Math
								.max(SVMCommon.get_runtime() - rt2, 0);
				}
				...
				if (SVMStructCommon.struct_verbosity >= 2)
					rt_opt += Math.max(SVMCommon.get_runtime() - rt2, 0);
		...
		if (SVMStructCommon.struct_verbosity >= 1) {
			...
			for (i = 0; i < cset.m; i++)
				alphasum += alpha_g[i] * cset.rhs[i];
			if (kparm.kernel_type ==  ModelConstant.LINEAR)
				modellength = SVMCommon.model_length_n(svmModel);
			else
				modellength = SVMCommon.model_length_s(svmModel);
			...
			int mininactive)
	{
		...
		if (cset.m != m) {
			cset.m = m;
			SVMCommon.realsmallloc_lhs(cset);
			SVMCommon.realsmallloc_rhs(cset);

			alpha_g = SVMCommon.reallocDoubleArr(alpha_g, cset.m);
			alphahist_g = SVMCommon.reallocIntArr(alphahist_g, cset.m);
	...
	public MATRIX init_kernel_matrix(CONSTSET cset, KERNEL_PARM kparm)
	{
		...
		matrix = SVMCommon.create_matrix(i + 50, i + 50);

		for (j = 0; j < cset.m; j++) {
			for (i = j; i < cset.m; i++) {
				kval = SVMCommon.kernel(kparm, cset.lhs[j], cset.lhs[i]);
			...
			int n, STRUCTMODEL sm, STRUCT_LEARN_PARM sparm)
	{
		...
		if (SVMStructCommon.struct_verbosity >= 2)
			rt_viol_g += Math.max(SVMCommon.get_runtime() - rt2, 0);
		...
		if (SVMStructCommon.struct_verbosity >= 2)
			rt2 = SVMCommon.get_runtime();
		if (fycached != null)
			fy = SVMCommon.copy_svector(fycached);
		else
			fy = ssa.psi(ex.x, ex.y, sm, sparm);
		fybar = ssa.psi(ex.x, ybar, sm, sparm);
		if (SVMStructCommon.struct_verbosity >= 2)
			rt_psi_g += Math.max(SVMCommon.get_runtime() - rt2, 0);
		...
		SVMCommon.mult_svector_list(fy, factor);
		SVMCommon.mult_svector_list(fybar, -factor);
		SVMCommon.append_svector_list(fybar, fy); // compute fy-fybar 
			...
			CONSTSET cset, KERNEL_PARM kparm)
	{
		...
		used = SVMCommon.create_nvector(maxkernelid + 2);
		SVMCommon.clear_nvector(used, maxkernelid + 2);
		for (i = 0; i < cset.m; i++)
			if (i != newpos)
		...
		if ((matrix == null) || (maxkernelid >= matrix.m))
			matrix = SVMCommon.realloc_matrix(matrix, maxkernelid + 50,
					maxkernelid + 50);

		for (i = 0; i < cset.m; i++) {
			kval = SVMCommon.kernel(kparm, cset.lhs[newpos], cset.lhs[i]);
