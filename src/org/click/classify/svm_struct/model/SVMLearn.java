...
import org.click.classify.svm_struct.model.SVMCommon;
...
public class SVMLearn {
	...
	public void svm_learn_classification(DOC[] docs, double[] classc,
		...
		runtime_start = SVMCommon.get_runtime();
		...
		if (alpha != null) {
			...
			SVMCommon.clear_nvector(weights, totwords);// set weights to zero
		...
		if (SVMCommon.verbosity >= 1) {
			...
			if ((SVMCommon.verbosity >= 1)
				...
				if ((learn_parm.remove_inconsistent == 0)
						&& (transduction == 0)) {
					runtime_start_xa = SVMCommon.get_runtime();
					if (SVMCommon.verbosity >= 1) {
					...
					System.out
							.println("Runtime for XiAlpha-estimates in cpu-seconds: "
									+ (SVMCommon.get_runtime() - runtime_start_xa)
									/ 100.0);
		...
		if (learn_parm.compute_loo != 0) {
			...
			runtime_start_loo = SVMCommon.get_runtime();
			for (i = 0; i < totdoc; i++) {
			...
			if (SVMCommon.verbosity >= 1) {
				...
				System.out.println("Runtime for leave-one-out in cpu-seconds: "
						+ (SVMCommon.get_runtime() - runtime_start_loo)
						/ 100.0);
			}
		}
	...
	public void svm_learn_optimization(DOC[] docs, double[] rhs, int totdoc,
		...
		if (alpha != null) {
			...
			if (kernel_parm.kernel_type ==  ModelConstant.LINEAR) {
				weights = new double[totwords + 1];
				SVMCommon.clear_nvector(weights, totwords);
				aicache = null;
			} else {
				weights = null;
				aicache = new double[totdoc];
			...
			if (cache != null) {
				...
				for (j = 0; j < kernel_cache.activenum; j++) {
					...
					if ((kernel_cache.index[k] != -1) && (l != -1) && (k != m)) {
						...
						cache[j] = SVMCommon.kernel(kernel_parm, ex, docs[k]);
					}
		...
		if ((kernel_cache != null) && (kernel_cache.index[docnum] != -1)) {
			...
			for (i = 0; (j = active2dnum[i]) >= 0; i++) {
				if (kernel_cache.totdoc2active[j] >= 0) {
					...
					buffer[j] = SVMCommon.kernel(kernel_parm, ex, docs[j]);
				}
			}
		} else {
			for (i = 0; (j = active2dnum[i]) >= 0; i++) {
	...
	public int optimize_to_convergence(DOC[] docs, int[] label, int totdoc,
		...
		for (; (retrain != 0) && (terminate == 0); iteration++) {
			...
			if ((retrain == 0)
				...
				if (((SVMCommon.verbosity >= 1) && (kernel_parm.kernel_type !=  ModelConstant.LINEAR))
			...
			if ((retrain == 0) && (learn_parm.remove_inconsistent != 0)) {
				...
				if (retrain != 0) {
					if (kernel_parm.kernel_type ==  ModelConstant.LINEAR) { // reinit shrinking
		...
		for (i = 0; i < totdoc; i++) {
			if (unlabeled[i] != 0) {
				...
				if ((learn_parm.rho * a[i] * r_delta_sq + xi) >= 1.0) {
					...
					if ((learn_parm.xa_depth == 0)
							|| ((a[i]
									* SVMCommon.kernel(kernel_parm, docs[i],
	...
	public static KERNEL_CACHE kernel_cache_init(int totdoc, int buffsize) {
		...
		if (SVMCommon.verbosity >= 2) {
			...
			System.out.println(" Kernel evals so far: "
					+ SVMCommon.kernel_cache_statistic);
		}
