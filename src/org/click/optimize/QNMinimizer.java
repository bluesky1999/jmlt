 ...
 * </code></blockquote> mem - the number of previous estimate vector pairs to
...
public class QNMinimizer implements Minimizer<DiffFunction>, HasEvaluators {
	...
	public class QNInfo {
		...
		public void setHistory(List<double[]> sList, List<double[]> yList) {
			...
			for (int i = 0; i < size; i++) {
				update(sList.get(i), yList.get(i),
						ArrayMath.innerProduct(yList.get(i), yList.get(i)),
						ArrayMath.innerProduct(sList.get(i), yList.get(i)), 0,
						1.0);
			}
		}
	...
	private void computeDir(double[] dir, double[] fg, QNInfo qn)
		...
		for (int i = mmm - 1; i >= 0; i--) {
			// alpha(i)<-rho(i)*<s(i),q>, qn->S(i),qb->Y(i)?
			// ////secant pairs s = x - previousX and y = grad - previousGrad.
			// LoggerUtil.logArray(logger, qn.getS(i), 100);
	...
	public float[] minimize(DiffFloatFunction function,
			float functionTolerance, float[] initial) {
	...
	public double[] minimize(DiffFunction dfunction, double functionTolerance,
		...
		if (outputToFile) {
			try {
				...
				infoFile = new PrintWriter(new FileOutputStream(baseName
						+ ".info"), true);
				infoFile.println(dfunction.domainDimension()
						+ "; DomainDimension ");
				infoFile.println(mem + "; memory");
			} catch (IOException e) {
				throw new RuntimeIOException(
						"Caught IOException outputting QN data to file", e);
			}
		}
		...
		do {
			...
			try {
				...
				boolean doEval = (its > 0 && evaluateIters > 0 && its
						% evaluateIters == 0);
				...
				// LoggerUtil.logArray(logger, dir, 100);
				...
				// x = newX;
				// newX = temp;
				// set x and grad to newx and newGrad
			...
			/*
			 * int all=0; int allnz=0; for(int i=0;i<newX.length;i++) {
			 * if(Math.abs(newX[i])<1.0E-5) { allnz++; } }
			 * 
			 * all=newX.length;
			 * 
			 * System.out.println("all:"+all);
			 * System.out.println("allnz:"+allnz);
			 * System.out.println("proo:"+(double)allnz/(double)all);
			 * logger.info("proo:"+(double)allnz/(double)all);
			 */
	...
	private double evaluateFunction(DiffFunction dfunc, double[] x,
			double[] grad) {
		// 将E[f]-Ep[f]拷贝到grad�?
		System.arraycopy(dfunc.derivativeAt(x), 0, grad, 0, grad.length);
		fevals += 1;
		// compute the value of log likelihood function at x
		// CRFLogConditionalObjectiveFunction.valueAt(x)
	...
	private double[] lineSearchBacktrack(Function func, double[] dir,
		...
		// System.err.println("normGradInDir:"+normGradInDir);
		...
		// System.err.println("its:"+its+" step:"+step);
		fevals += 1;
		if (fevals > maxFevals) {
			throw new MaxEvaluationsExceeded(
	...
	private double[] lineSearchMinPack(DiffFunction dfunc, double[] dir,
		...
		bracketed = false;
		boolean stage1 = true;
		System.err.println("aMax:" + aMax + " aMin:" + aMin);
	...
	private int getStep(
	/*
	 * double[] x, double[] dir, double[] newX, double f0, double g0, // None of
	 * these were used
	 */
	double[] newPt, double[] bestPt, double[] endPt, double stpMin,
			double stpMax) throws MaxEvaluationsExceeded {
	...
	// newPoint[a] = aLin; // Guess low at first since we have no idea of scale
	// at
	// first.
	// }
	//
	...
	// if( (newPoint[f] > f0 + ftol*newPoint[a]*g0) || newPoint[f] >
	// prevPoint[f]
	// ){
	// //We know there must be a point that satisfies the strong wolfe
	// conditions
	// between
	...
	// //we should increase our step size, since we have potential to decrease
	// the
	// function
	// //value a lot more.
	...
	// if(fevals > maxFevals){ throw new MaxEvaluationsExceeded(" Exceeded
	// during
	// zoom() Function ");}
	//
	// if(newPoint[a] > aMax){
	 ...
	 * 
	 * if( Double.isNaN(pointList.get(0)[gInd]) ){ // We don't know the gradient
	 * at aLow so do bisection newAlpha = 0.5*(pointList.get(0)[aInd] +
	 * pointList.get(1)[aInd]); }else{ // We know the gradient so do Quadratic
	 * 2pt newAlpha =
	 * interpolateQuadratic2pt(pointList.get(0),pointList.get(1)); }
	 * 
	...
	// }
	// private double interpolateCubic(double[] pt0, double[] pt1, double[]
	// pt2){
	...
	// double[] newX, double f0, double g0, double[] bestPoint, double[]
	// endPoint)
	// throws MaxEvaluationsExceeded {
	...
	// //the decrease in gradient. if the new gradient is telling us >0 we need
	// to
	// look behind us
